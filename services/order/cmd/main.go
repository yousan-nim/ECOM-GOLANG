// Command order-service is the entry point (composition root) for the
// order microservice: load config → open DB → run migrations → serve HTTP.
package main
...
import (
	...
	order "github.com/yousan-nim/ecom/services/order"
...
func main() {
	...
	if err := db.Migrate(sqlDB, order.MigrationsFS); err != nil {
